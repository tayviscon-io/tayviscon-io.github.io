...
_В этой статье мы рассмотрим [Judge 0](https://github.com/judge0/judge0),
чтобы лучше понять основные концепции разработки надежных и масштабируемых систем онлайн-исполнения кода,
...
Представленный в Октябре 2017,
Judge0 на данный момент активно используется и является важнейшей частью нескольких действующих приложений.
...
современную архитектуру таких систем на примере Judge0.
Наконец, в пятом разделе мы приведем краткий обзор смежных работ, выполненных в области систем онлайн-исполнения кода.
...
## ОБЗОР ОСНОВНЫХ КОНЦЕПЦИЯ, ПРИМЕНЯЕМЫХ ПРИ РАЗРАБОТКЕ Judge0 

Judge0 - это новая и открытая система онлайн-исполнения кода,
отличающаяся масштабируемой архитектурой и все чаще используют новые подходы в создании систем онлайн-судейства.
Разработка Judge0 началась в Августе 2016 года и ставила перед собой цель:
...
На пути разработки Judge0 встретился большой ряд технических проблем,
которые мы рассмотрим подробнее в следующих пунктах.

### Масштабируемость

Масштабируемость Judge0 может быть достигнута с помощью трех стратегий.
...
Judge0 имеет модульную архитектуру, где каждый модуль может быть легко развернут в виде контейнера Docker,
что позволяет масштабировать только те части системы, которые могут стать узким местом.

### Безопасность и песочница

Поскольку Judge0 позволяет пользователям запускать произвольный исходный код и двоичные файлы
...
Движок Judge0 использует *isolate* (см. рисунок 3) для песочницы компиляции и исполнения. *Isolate* - это хорошо
...
Judge0 имеет богатые возможности конфигурации для ограничения ресурсов путем
...
Используя *isolate* в качестве среды песочницы, Judge0 может
безопасно компилировать и исполнять произвольный исходный код и даже двоичные файлы,
полученные из неизвестного источника. Более того, разбирая метаданные выполнения из *isolate* движок Judge0
...
Judge0 должна быть разработана таким образом,
...
Judge0 может, например, позволять пользователям запрашивать Base64 закодированные данные в формате JSON.
...
Judge0 имеет простой JSON веб-интерфейс, который имеет две основные точки:
...
Системы онлайн-исполнения кода имеет масштабируемую архитектуру, как показано на рисунках 2 и 3.
Более детальное взаимодействие между компонентами системы показано на рисунке 4.

![Диаграмма последовательности работы Judge0](online-code-execution-system-sequence-diagram.png)
_Рисунок 4: Диаграмма последовательности работы Judge0_
...
который используется в Judge0. В заключение следует отметить,
что обе эти системы ориентированы на соревновательное программирование,
в то время как Judge0 представляет собой решение общего назначения.
...
В данной статье мы рассказали о новой системе Judge0,
